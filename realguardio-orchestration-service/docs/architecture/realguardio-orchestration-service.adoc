...
** Request Body: `{"customerId": Long, "name": String, "locationName": String}`
** Returns: `CreateSecuritySystemResponse` with `securitySystemId`
** Process: Asynchronously orchestrates creation across services
...
** `CreateSecuritySystemCommand` - Create security system entity
** `NoteLocationCreatedCommand` - Link location to security system
** `UpdateCreationFailedCommand` - Mark creation as failed (compensation)

* *Customer Service*:
** `CreateLocationWithSecuritySystemCommand` - Create location linked to security system
...
*Purpose*: Coordinates creation of a security system and its associated location across two services.
...
* Current step
* Saga data (customerId, locationName, securitySystemId, locationId)
* Completion status
