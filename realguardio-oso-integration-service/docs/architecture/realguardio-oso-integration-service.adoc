...
==== Security System Service Events

The `SecuritySystemEventConsumer` class handles these domain events:

* `SecuritySystemAssignedToLocation` - Links security system to location
** Handler: `SecuritySystemEventConsumer.handleSecuritySystemAssignedToLocation()`
** Channel: `io.eventuate.examples.realguardio.securitysystemservice.domain.SecuritySystem`
