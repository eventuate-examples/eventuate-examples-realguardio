...
function makeNewToken(tokensOrError, oldToken: JWT) {
...
async function refreshToken(token: JWT) {
  ...
  try {
    const response = await fetchNewTokens(token.refreshToken! as string);
    ...
    return makeNewToken(tokensOrError, token);
  } catch (error) {
